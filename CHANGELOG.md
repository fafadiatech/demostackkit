...
- **Job Card Inspection Checklists** — 11 Manufacturing industries' `01_master/08_operations.py` now cache which Operation is their QC gate (e.g. "Solar QC Sign-off", "QC Inspection", "Quality Testing") plus its inspection parameters; new shared `01_master/95_quality_inspection_templates.py` builds a `Quality Inspection Template` from those parameters and links it onto that Operation. `02_transactions/215_production.py` then auto-creates an In Process `Quality Inspection` (linked back via `reference_type`/`reference_name` and the Job Card's own `quality_inspection` field) whenever a completed Job Card's Operation carries a template, with an 85% pass rate matching the existing per-industry inspection seeders, and assigns it to one of the company's seeded employee logins via `frappe.desk.form.assign_to`. The 11 per-industry `03_quality_inspections.py` seeders no longer generate "In Process" inspections, since those are now created and linked by `215_production.py` (mirrors the "Incoming" removal from ref #35). (ref #1)
