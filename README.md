...
- [Batch & Serial No Traceability](#batch--serial-no-traceability)
...
Every industry's data is generated by a pipeline of small, composable **seeders** rather than a fixture dump — so the same data model an EPC company demo needs (Projects, milestone billing) is also available to an EV manufacturer (Work Orders, Job Cards) without either industry's config having to know the other exists.

**Where seeders live:**

- `industries/<slug>/seeders/{01_master,02_transactions}/` — industry-specific: this industry's own items, BOMs, project blueprints, quality parameters.
- `demostackkit/seeders/{01_master,02_transactions}/` — **shared**: runs for *every* industry that meets a module/volume gate (Opening Stock, Purchase Receipts, Production, Delivery Notes, Payroll, Projects, Maintenance, ...). This is what lets one new capability — like [Batch & Serial No Traceability](#batch--serial-no-traceability) — light up across all 11 Manufacturing industries at once, from a single new file.

`demostackkit/seeder/loader.py` discovers and merges both sets for a given industry automatically; nothing has to be registered by hand.

**Master vs. Transaction seeders:**

| | Master (`BaseMasterSeeder`) | Transaction (`BaseTransactionSeeder`) |
| --- | --- | --- |
| Examples | Company, Items, Warehouses, BOM | Sales Orders, Purchase Receipts, Stock Entries |
| Idempotent? | Yes — always checks `frappe.db.exists()` before inserting | No — `demostackkit reset` recreates the site, so duplicates never matter |
| Default priority | 10 | 200 |
| Randomness | rarely needed | must use `self.ctx.random`, never Python's global `random` module |

**Ordering:** every master seeder runs before every transaction seeder; within a phase, seeders run in ascending `priority` order (ties broken alphabetically by filename). A seeder declares its own `priority` as a class attribute — there's no separate dependency graph, so a seeder that reads BOM data simply picks a priority number after the BOM seeder's.

**How a seeder actually creates data:** rather than talking to ERPNext over its REST API, each seeder builds a Python script as a string — with any input data JSON-embedded into it — and runs that script with `docker exec` inside the ERPNext backend container (`demostackkit/seeder/base.py::BaseSeeder._exec`), using the real `frappe`/`erpnext` modules directly (`frappe.get_doc(...).insert()/.submit()`). This means seeders get to reuse ERPNext's own internal helpers — `make_purchase_receipt()`, `make_delivery_note()`, `Production Plan.make_work_order()` — the exact same document-mapping logic the ERPNext UI's buttons call, instead of reimplementing it.

**Cross-seeder handoff:** a shared `SeedContext.cache_set()`/`cache_get()` lets one seeder pass data to a later one in the same run — e.g. the Item seeder caches which item codes are finished goods so the Production seeder doesn't have to re-derive it.

**Determinism:** `demostackkit reset garment` always produces byte-for-byte identical data because every seeder draws randomness from `self.ctx.random`, a `random.Random` instance seeded from `industry.yaml`'s `seed.random_seed` — never Python's unseeded global `random` module. See [Deterministic Resets](#deterministic-resets).
...
Manufacturing industries also seed **forward and backward traceability**: a raw-material lot can be traced forward through a Work Order to the finished good it became and on to the customer it shipped to, and a shipped unit can be traced backward to the exact raw-material lots consumed to build it — using ERPNext's own native Batch/Serial ledger and reports (Stock Ledger filtered by batch, Batch-Wise Balance History), not a bespoke screen.

A shared master seeder (`86_batch_tracking.py`) discovers every item referenced by a submitted default BOM and flags it:

- **Components / sub-assemblies** get `has_batch_no` + `create_new_batch` + `batch_number_series` — ERPNext then auto-creates a new Batch on every *incoming* stock movement (Purchase Receipt, Work Order Manufacture output) with no further code.
- **Top-level finished goods** get `has_serial_no` + `serial_no_series` instead by default — an individually-identifiable end product (VIN-style) — unless `serialize_top_level_fg: false`, in which case they're batch-tracked like everything else (used by industries that ship lots, not individually-serialized units: `chemical`, `abrasives`, `crockery`, `garment`, `ingredientmfg`, `print3d`).

*Outward* movements (Work Order material consumption, Delivery Note shipment) never fabricate a lot — the Production and Delivery Note seeders explicitly select a real, already-in-stock batch/serial via FIFO (or FEFO, `based_on: "Expiry"`), the same mechanism the "Add Serial / Batch No" UI dialog uses. Receiving raw materials also stamps a cosmetic Vendor Batch identifier + the supplier link onto every auto-created Batch.

```yaml
seed:
  batch_tracking:
    enabled: true            # false = no batch/serial tracking (default)
    serialize_top_level_fg: true   # false = batch-track finished goods too
    based_on: "FIFO"          # or "Expiry" (FEFO)
```

Enabled for all 11 Manufacturing-module industries. Skipped entirely when Manufacturing is not in `modules` or `batch_tracking.enabled` is false.

| Priority | Seeder | |
| --- | --- | --- |
| 86 | `01_master/86_batch_tracking.py` | flags BOM-linked items with Batch/Serial No |
| 211 | `02_transactions/211_purchase_receipts.py` | stamps Vendor Batch + supplier on receipt |
| 215 | `02_transactions/215_production.py` | FIFO/FEFO batch selection on Work Order consumption |
| 220 | `02_transactions/220_delivery_notes.py` | FIFO/FEFO batch selection on shipment |
