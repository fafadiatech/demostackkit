...
"""
        ...
        self.ctx.cache_set("fg_item_codes", [row["item_code"] for row in items if row.get("item_group") in ("Finished Products",)])
        self.ctx.cache_set("rm_item_codes", [row["item_code"] for row in items if row.get("item_group") in ("Raw Chemicals", "Solvents", "Intermediates")])
        self.ctx.cache_set(
            "rm_items",
            [
                {
                ...
                for row in items
                if row.get("item_group") in ("Raw Chemicals", "Solvents", "Intermediates")
            ],
        )
