"""
Seeder: Purchase Orders for PowerTech Electrical Manufacturing.

Generates purchase orders for raw materials (copper winding wire, CRGO silicon
steel, transformer oil, insulation paper, MS sheet, bushings, breakers, relays,
busbars, and packaging) from the supplier list. Each PO contains 1-4 randomly
...
class PurchaseOrderSeeder(BaseTransactionSeeder):
    ...
    def run(self) -> None:
        ...
        for i in range(self.volume):
            ...
            items = []
            for rm in chosen:
                rate = round(rm["valuation_rate"] * rng.uniform(0.90, 1.10), 2)
                items.append(
                    {
