"""
Seeder: Purchase Orders for Nexus TCG & Hobbies.

Generates purchase orders from TCG distributors for sealed product inventory
(booster boxes and cases for Pokémon, MTG, and Yu-Gi-Oh!) and raw singles
purchased for resale. Each PO targets one of the four major distributors with
1-4 randomly chosen product lines. Quantities are 1-10 units (or cases) with
rates derived from the item's valuation_rate with ±10% market variance to
simulate real distributor pricing fluctuations.

Uses deterministic random (self.ctx.random) for reproducibility.
Running `demostackkit reset hobbytcg` always produces identical POs.
"""
...
class PurchaseOrderSeeder(BaseTransactionSeeder):
    ...
    def run(self) -> None:
        ...
        for i in range(self.volume):
            ...
            items = []
            for rm in chosen:
                rate = round(rm["valuation_rate"] * rng.uniform(0.90, 1.05), 2)
                items.append(
                    {
