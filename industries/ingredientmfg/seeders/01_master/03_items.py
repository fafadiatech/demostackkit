...
"""
        ...
        self.ctx.cache_set(
            ...
            [
                ...
                for row in items
                if row.get("item_group")
                in ("Raw Botanicals", "Precursor Chemicals", "Intermediates")
            ],
        )
        self.ctx.cache_set(
            "rm_items",
        ...
        self.ctx.cache_set(
            "fg_additive_item_codes",
            [
                row["item_code"]
                for row in items
                if row.get("item_group") == "Flavours, Colours & Functional Additives"
            ],
        )
        self.ctx.cache_set(
            "fg_nutraceutical_item_codes",
            [
                row["item_code"]
                for row in items
                if row.get("item_group") == "Nutraceutical & Botanical Actives"
            ],
        )
