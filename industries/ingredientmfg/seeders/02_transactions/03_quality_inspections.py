"""
Seeder: Quality Inspections for Ingredient Manufacturing.

Creates Incoming, In Process, and Outgoing Quality Inspections across
ingredient items, covering purity, moisture, active-compound assay,
microbial load and heavy metals screening. 85% pass rate for realistic
demo data.
"""
...
_QI_PARAMETERS = [
    ("Purity %", 95, 99.9),
    ("Moisture Content %", 0, 6.0),
    ("Active Compound Assay %", 90, 105),
    ("Microbial Load (CFU/g)", 0, 1000),
    ("Heavy Metals (ppm)", 0, 10),
]
        ...
        for _ in range(self.volume):
            ...
            for i, (spec, min_val, max_val) in enumerate(chosen_params):
                if accepted or i < n_params - 1:
                    reading_val = round(rng.uniform(min_val, max_val), 4)
                    r_status = "Accepted"
                else:
                    reading_val = round(rng.uniform(max_val * 1.05, max_val * 1.2), 4)
                    r_status = "Rejected"
                readings.append(
                    {
                        "specification": spec,
