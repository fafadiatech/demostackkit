...
class PurchaseOrderSeeder(BaseTransactionSeeder):
    ...
    def run(self) -> None:
        ...
        for _ in range(self.volume):
            ...
            # Bulk commodity buying is shipping/contract driven — longer lead
            # times than a typical FMCG distribution reorder.
            required_date = order_date + timedelta(days=rng.randint(21, 60))
            supplier = rng.choice(suppliers)
            chosen = rng.sample(rm_items, min(rng.randint(1, 3), len(rm_items)))
            items = [
                {
                    "item_code": rm["item_code"],
                    # Larger default order quantities — commodity lots, not
                    # retail case packs.
...
for o in orders:
    try:
        po = frappe.get_doc({{
            ...
            'items': [{{'item_code': it['item_code'], 'qty': it['qty'], 'rate': it['rate'], 'schedule_date': it['schedule_date'], 'uom': it['uom'], 'stock_uom': it['uom'], 'conversion_factor': 1}} for it in o['items']],
        }})
