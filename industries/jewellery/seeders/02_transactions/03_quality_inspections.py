"""
Seeder: Quality Inspections for Jewellery Manufacturing.

Creates Incoming, In Process, and Outgoing Quality Inspections
across jewellery items. 85% pass rate for realistic demo data.
"""
        ...
        for _ in range(self.volume):
            ...
            for i, (spec, min_val, max_val) in enumerate(chosen_params):
                if accepted or i < n_params - 1:
                    reading_val = round(rng.uniform(min_val, max_val), 3)
                    r_status = "Accepted"
                else:
                    reading_val = round(rng.uniform(max_val * 1.05, max_val * 1.2), 3)
                    r_status = "Rejected"
                readings.append(
                    {
                        "specification": spec,
