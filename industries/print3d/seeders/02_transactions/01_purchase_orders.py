"""
Seeder: Purchase Orders for 3D Printing Services.

Generates purchase orders for raw materials (filaments, resins, post-processing
supplies, and packaging) from the supplier list. Each PO contains 1-4 randomly
chosen raw material lines with quantities between 5-100 units and rates derived
from the item's valuation_rate (with ±15% market variance).

Uses deterministic random (self.ctx.random) for reproducibility.
Running `demostackkit reset print3d` always produces identical POs.
"""
...
class PurchaseOrderSeeder(BaseTransactionSeeder):
    ...
    def run(self) -> None:
        ...
        for i in range(self.volume):
            ...
            items = []
            for rm in chosen:
                rate = round(rm["valuation_rate"] * rng.uniform(0.9, 1.15), 2)
                items.append(
                    {
